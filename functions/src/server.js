...
module.exports = init = (async () => {
    ...
    Object.keys(urls).forEach((path) => {
        ...
        try {
            ...
            handlers.forEach((handler) =>
                router[handler.type ? handler.type : 'get'](`${path}${handler.url ? handler.url : ''}`, csrfProtection, handler.exec ? handler.exec : handler));
            logger.info(path);
        } catch (e) {
            console.log(e);
    ...
    server
        ...
        .use(session({
            store: new FirebaseStore({database: require(`./firebase`).admin.database()}),
            secret: require(`${utils.baseDir}/config/server.json`)['sessionSecret'],
            resave: true,
